...
keywords["job-creation-attributes-supported"]=			setof_keyword([
	"&lt;Any Job Template attribute keyword name&gt;",
...
//client side:
 var obj = {};
 $('#table-ipp-registrations-4 tbody tr').each(function(tr){
 var $tr = $(this);
 var $tds = $tr.find("td");
 //if(!$tds[4].innerHTML) return;
 var attr = $tds[0].innerHTML;

 var value = $tds[1].innerHTML;
 var syntax = $tds[2].innerHTML;
 var type = $tds[3].innerHTML;
 if(!obj[attr]) {
 obj[attr]=[];
 }
 obj[attr].push([value||"", syntax, type]);
...
//server
var map = {
	"type3 keyword":						"keyword",
	"1setOf type3 keyword":					"setof_keyword",
	"1setOf type2 keyword":					"setof_keyword",
	"type2 keyword":						"keyword",
	"type3 keyword | name(MAX)":			"keyword_name",
	"(type3 keyword | name(MAX))":			"keyword_name",
	"1setOf (type3 keyword | name(MAX))":	"setof_keyword_name",
	"type2 keyword | name(MAX)":			"keyword_name",
	"no-value | type3 keyword | name(MAX)":	"keyword_name_novalue",
	"1setOf (type3 keyword | name(MAX)":	"setof_keyword_name",
	"1setOf (type2 keyword)":				"setof_keyword",
	"1setof (type2 keyword | name(MAX))":	"setof_keyword_name"
};

var refd = /Any "([^"]+)" keyword value/;
var t = "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t";
var media = {};
console.log('var keywords = {};');
Object.keys(x).forEach(function(name1,i,arr1){
	var attr = x[name1],
		declaration = 'keywords["'+name1+'"]=',
		dlength = (declaration.length/4)|0,
		tabs = t.substr(-(14-dlength)), referencing;
	process.stdout.write(declaration +tabs+ map[attr[0][1]]+'(');
	...
	attr.forEach(function(value){
		...
		if(name1=="media"){
			if(!media[value[2]])
				media[value[2]] = [];
			media[value[2]].push(value[0]);
		}
		referencing = refd.exec(value[0]);
		if(referencing){
			console.log('\n\tref("'+referencing[1]+'")\n);');
		}
	});
	...
	var values = attr.map(function(value){ return value[0];}).filter(empties);
	process.stdout.write(JSON.stringify(values,null, '\t'));
	console.log(');');
});
console.log("var media = "+JSON.stringify(media, null, '\t'));
function empties(value){ return !!value;}
//console.log('};');
